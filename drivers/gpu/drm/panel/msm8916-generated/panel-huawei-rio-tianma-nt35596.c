...
static inline
struct tianma_nt35596_5p5 *to_tianma_nt35596_5p5(struct drm_panel *panel)
{
	return container_of(panel, struct tianma_nt35596_5p5, panel);
}

static void tianma_nt35596_5p5_reset(struct tianma_nt35596_5p5 *ctx)
{
...
static int tianma_nt35596_5p5_on(struct tianma_nt35596_5p5 *ctx)
{
	...
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, MIPI_DCS_WRITE_CONTROL_DISPLAY,
				     0x24);
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, MIPI_DCS_WRITE_POWER_SAVE, 0x91);
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, MIPI_DCS_SET_CABC_MIN_BRIGHTNESS,
				     0x28);
	mipi_dsi_dcs_set_display_brightness_multi(&dsi_ctx, 0x0000);
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0xd3, 0x05);
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0xd4, 0x07);
...
static int tianma_nt35596_5p5_off(struct tianma_nt35596_5p5 *ctx)
{
	struct mipi_dsi_multi_context dsi_ctx = { .dsi = ctx->dsi };
...
static int tianma_nt35596_5p5_prepare(struct drm_panel *panel)
{
	struct tianma_nt35596_5p5 *ctx = to_tianma_nt35596_5p5(panel);
	struct device *dev = &ctx->dsi->dev;
	int ret;

	tianma_nt35596_5p5_reset(ctx);

	ret = tianma_nt35596_5p5_on(ctx);
	if (ret < 0) {
...
static int tianma_nt35596_5p5_unprepare(struct drm_panel *panel)
{
	struct tianma_nt35596_5p5 *ctx = to_tianma_nt35596_5p5(panel);
	struct device *dev = &ctx->dsi->dev;
	int ret;

	ret = tianma_nt35596_5p5_off(ctx);
	if (ret < 0)
...
static const struct drm_display_mode tianma_nt35596_5p5_mode = {
	.clock = (1080 + 108 + 8 + 96) * (1920 + 7 + 2 + 3) * 60 / 1000,
	.hdisplay = 1080,
	.hsync_start = 1080 + 108,
	.hsync_end = 1080 + 108 + 8,
	.htotal = 1080 + 108 + 8 + 96,
	.vdisplay = 1920,
	.vsync_start = 1920 + 7,
	.vsync_end = 1920 + 7 + 2,
	.vtotal = 1920 + 7 + 2 + 3,
...
static int tianma_nt35596_5p5_get_modes(struct drm_panel *panel,
					struct drm_connector *connector)
{
	return drm_connector_helper_get_modes_fixed(connector, &tianma_nt35596_5p5_mode);
}

static const struct drm_panel_funcs tianma_nt35596_5p5_panel_funcs = {
	.prepare = tianma_nt35596_5p5_prepare,
	.unprepare = tianma_nt35596_5p5_unprepare,
	.get_modes = tianma_nt35596_5p5_get_modes,
};

static int tianma_nt35596_5p5_bl_update_status(struct backlight_device *bl)
{
...
static int tianma_nt35596_5p5_bl_get_brightness(struct backlight_device *bl)
{
	struct mipi_dsi_device *dsi = bl_get_data(bl);
	u16 brightness;
	int ret;
...
static const struct backlight_ops tianma_nt35596_5p5_bl_ops = {
	.update_status = tianma_nt35596_5p5_bl_update_status,
	.get_brightness = tianma_nt35596_5p5_bl_get_brightness,
};

static struct backlight_device *
tianma_nt35596_5p5_create_backlight(struct mipi_dsi_device *dsi)
{
	...
	return devm_backlight_device_register(dev, dev_name(dev), dev, dsi,
					      &tianma_nt35596_5p5_bl_ops, &props);
}

static int tianma_nt35596_5p5_probe(struct mipi_dsi_device *dsi)
{
	struct device *dev = &dsi->dev;
	struct tianma_nt35596_5p5 *ctx;
	int ret;
	...
	drm_panel_init(&ctx->panel, dev, &tianma_nt35596_5p5_panel_funcs,
		       DRM_MODE_CONNECTOR_DSI);
	ctx->panel.prepare_prev_first = true;

	ctx->panel.backlight = tianma_nt35596_5p5_create_backlight(dsi);
...
static void tianma_nt35596_5p5_remove(struct mipi_dsi_device *dsi)
{
	struct tianma_nt35596_5p5 *ctx = mipi_dsi_get_drvdata(dsi);
	int ret;
...
static const struct of_device_id tianma_nt35596_5p5_of_match[] = {
	{ .compatible = "huawei,rio-tianma-nt35596" },
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, tianma_nt35596_5p5_of_match);

static struct mipi_dsi_driver tianma_nt35596_5p5_driver = {
	.probe = tianma_nt35596_5p5_probe,
	.remove = tianma_nt35596_5p5_remove,
	.driver = {
		.name = "panel-huawei-rio-tianma-nt35596",
		.of_match_table = tianma_nt35596_5p5_of_match,
	},
};
module_mipi_dsi_driver(tianma_nt35596_5p5_driver);

MODULE_AUTHOR("linux-mdss-dsi-panel-driver-generator <fix@me>"); // FIXME
MODULE_DESCRIPTION("DRM driver for TIANMA_NT35596_5P5_1080P_VIDEO");
MODULE_LICENSE("GPL");
